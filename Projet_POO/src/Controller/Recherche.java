...
public class Recherche {
 ...
 /*----------------------------------------------------------------------------------------------------*/
    ...
    public ArrayList<Seance> recapitulatif_cours_etudiant(int id_utilisateur, java.util.Date d_debut, java.util.Date d_fin)
    {
        DBConnect conn = new DBConnect();
        Connection connect=conn.getCon(); 
        ...
        for(int i=0 ; i< listeSeance.size(); i++){
            ...
            //System.out.println("la semaine du "+i+" eme seance de la liste est egale a  : "+listeSeance.get(i).getSemaine());
            if(listeSeance.get(i).getDate().compareTo(d_fin) > 0 || listeSeance.get(i).getDate().compareTo(d_debut)< 0)
            {
                //listeSeance.remove(i);
                compteur++;
            }
